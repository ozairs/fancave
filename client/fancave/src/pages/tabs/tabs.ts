  ...
  constructor(
    ...
    private storage: Storage) {
      console.log("<< TabsPage");
        //(if tabs page is initially loaded), subscribe to login events for UI to display appropriate menu options

        //subscribe to login event
        this.events.subscribe('user:login', (evt) => {
          this.global.setLogin(true, evt);
        });
        //subscribe to logout event
        this.events.subscribe('user:logout', (evt) => {
          this.global.setLogin(false, null);
        });
        console.log(">> TabsPage");
  }
    ...
    console.log("<< ionViewDidLoad");
      ...
      //check if user has successfully login (needed to display login/logout menu)
        ...
        this.storage.get('user').then((session) => {
          if (session) {
            this.global.setLogin(true, session.user.name);
          }    
        });
      }
      console.log(">> ionViewDidLoad");
